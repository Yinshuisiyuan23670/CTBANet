...
class PAM_Module(Module):
    ...
    #Ref from SAGAN
    def __init__(self, in_dim,psp_size=(1,3,6,8)):
...
class CAM_Module(Module):
    ...
    def __init__(self, in_dim):
        ...
        self.chanel_in = in_dim
        self.conv1 = nn.Conv2d(self.chanel_in, 1, kernel_size=1, stride=1, padding=0)
        self.conv2 = nn.Conv2d(self.chanel_in, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self,x):
        ...
        proj_key = self.conv1(x).view(m_batchsize, 1, -1).permute(0, 2, 1)
        energy = torch.bmm(proj_query, proj_key)
        energy_new = torch.max(energy, -1, keepdim=True)[0].expand_as(energy)-energy
        attention = self.softmax(energy_new)
        proj_value = self.conv2(x).view(m_batchsize, 1, -1)
